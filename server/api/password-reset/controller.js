...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your rapid-dapp-backend account.<br>
        ...
        &mdash; rapid-dapp-backend Team
      `
      return sendMail({ toEmail: email, subject: 'rapid-dapp-backend - Password Reset', content })
    })
